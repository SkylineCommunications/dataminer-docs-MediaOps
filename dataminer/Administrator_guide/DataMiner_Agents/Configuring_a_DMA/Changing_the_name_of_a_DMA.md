...
Users of a DaaS system do not have access to the *DataMiner.xml* file and cannot configure an alias that way. However, for recently deployed DaaS systems<!--RN 45327-->, the *mode* attribute in *DataMiner.xml* is set to "manual" by default in DaaS systems, so changing the DMA name in System Center in Cube updates the alias instead of the server name.
