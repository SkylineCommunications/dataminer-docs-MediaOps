...
Users of a DaaS system do not have access to the *DataMiner.xml* file and cannot configure an alias that way. However, from DataMiner 10.6.0 [CU3]/10.6.6 onwards<!--RN 45327-->, the *mode* attribute in *DataMiner.xml* is set to "manual" by default in DaaS systems, so changing the DMA name in System Center in Cube updates the alias instead of the server name.
